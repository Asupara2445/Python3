...
class print_progress():
    def __init__(self, max_epoch, batch_size, data_size):
        iters_per_epoch = data_size // batch_size
        if data_size % batch_size != 0:iters_per_epoch += 1
        self._max_iter = iters_per_epoch * max_epoch
    ...
    def max_iter(self, new_max_iter):
        iters_per_epoch = self.data_size // self.batch_size
        if self.data_size % self.batch_size != 0:iters_per_epoch += 1
        self._max_iter = iters_per_epoch * new_max_iter
        self._max_epoch = new_max_iter
